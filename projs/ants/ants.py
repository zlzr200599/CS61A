...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        i = 0
        while next_place is not None and not isinstance(next_place, Hive):
            find_bee = random_or_none(next_place.bees)
            if self.min_range <= i <= self.max_range and find_bee is not None:
                return find_bee
            else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor -= amount
        for b in self.place.bees[:]:  # copy the list to prevent skip elements killed
