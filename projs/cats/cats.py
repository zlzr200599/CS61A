...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    selector = filter(select, paragraphs)
    for n, p in enumerate(selector):
        if n == k:
            return p
    return ''
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def is_topic(p):
        remove_punct_p = remove_punctuation(p)
        lower_p = lower(remove_punct_p)
        split_p_list = split(lower_p)
        for t in topic:
            if t in split_p_list:
                return True
        return False
    return is_topic
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if min_len == 0:
        return 0.0
    mask = [ 1 for t, r in zip(typed_words[:min_len], reference_words[:min_len]) if t == r]
    return 100*sum(mask) / len(typed_words)
    # END PROBLEM 3
...
def fastest_words(game):
    ...
    Word = all_words(game)
    argmin = lambda a_list: min(range(len(a_list)), key= lambda i: a_list[i])
    fastest_word = [[] for _ in players]
    for i in words:
        fast_player_id = argmin([times[j][i] for j in players])
        fastest_word[fast_player_id].append(Word[i])
    return fastest_word
    # END PROBLEM 10
